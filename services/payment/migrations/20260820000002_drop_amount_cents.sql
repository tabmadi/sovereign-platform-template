-- migrate:up
-- The CONTRACT half of the expand/contract that introduced `charges.amount`
-- (ADR-0300). The expand migration added the money columns and backfilled from
-- `amount_cents`; this removes the column now that nothing reads it.
--
...
alter table charges drop column amount_cents;
...
alter table charges add column amount_cents integer not null default 0;
update charges set amount_cents = (amount::numeric * 100)::integer;
