...
- **Nothing is bundled.** `local-path` is an installed component rather than a shipped one, and a deployment without a provider load balancer needs one chosen rather than inherited.
