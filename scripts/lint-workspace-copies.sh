...
# a cluster bring-up.
#
# GLOB members (`libs/ts/sdks/*`) are exempt: a glob that matches nothing is not an
# error to bun, and the SDKs are generated rather than committed.
set -euo pipefail
...
members="$(yq -r '.workspaces[]' -p json "$MANIFEST" 2>/dev/null | grep -v '\*' || true)"
