...
| **A paging receiver** | Alerts route to email and an unwired paging webhook, with the Watchdog on its own unwired heartbeat receiver. Nothing pages, so overnight detection is next working day ([ADR-0502](adr/0502-alerting-and-on-call.md)) | The project states an availability objective tighter than next-working-day. The objective and the receiver are one decision, not two |
