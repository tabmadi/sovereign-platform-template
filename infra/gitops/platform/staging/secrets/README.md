...
This directory is synced by the `staging-secrets` Application (the `secrets`
...
real cluster secrets, and the `staging` cluster key in `.sops.yaml` is a placeholder.
...
1. Generate the `staging` cluster age key and replace the `cluster_staging` placeholder in
...
2. Copy the skeleton below to `platform.enc.yaml`, fill in real values, and
   encrypt it in place: `sops --encrypt --in-place infra/gitops/platform/staging/secrets/platform.enc.yaml`.
   The `.sops.yaml` rule for this path encrypts only `data`/`stringData` values
   (the CR structure stays readable) to `cluster_staging` + engineers + ops-recovery.
